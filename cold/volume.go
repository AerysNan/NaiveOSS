...
func (v *Volume) reconstruct(dataShard int, parityShard int) error {
	v.m.RLock()
	needRepair := make([]int, 0)
	data := make([][]byte, dataShard+parityShard)
...
func (v *Volume) readFromBlocks(offset, length int64, config *Config) ([]byte, error) {
	...
	for {
		...
		if err != nil || fmt.Sprintf("%x", sha256.Sum256(content)) != block.Tag {
			err = v.reconstruct(config.DataShard, config.ParityShard)
			if err != nil {
				return nil, err
			}
		}
